...
@pytest.mark.parametrize("type_a", [*all_types])
def test_gathernd(type_a):
    data = np.array([[[0, 1], [2, 3]], [[4, 5], [6, 7]]], dtype=type_a)
    indices = np.array([[[0, 1]], [[1, 0]]], dtype=np.int64)
    expected = np.array([[[2, 3]], [[4, 5]]], dtype=type_a)

    result = onp.nn.gathernd(onp.array(data), onp.array(indices))

    expect(expected, result.numpy())


@pytest.mark.parametrize("type_a", [*all_types])
def test_gathernd_batchdim_1(type_a):
    data = np.array([[[0, 1], [2, 3]], [[4, 5], [6, 7]]], dtype=type_a)
    indices = np.array([[1], [0]], dtype=np.int64)
    expected = np.array([[2, 3], [4, 5]], dtype=type_a)

    result = onp.nn.gathernd(onp.array(data), onp.array(indices), batch_dims=1)
